...
Imatges: `src/lib/assets.ts` (`imageForKey`, `imageObjectPositionForKey`). Temes populars / hero: `topic-*.jpg`, `habitatge.png`, `card-*.jpg`, `logo.png` (sync Lovable). **Destacats** («Potser t'has perdut»): il·lustracions úniques [Storyset](https://storyset.com) recolorejades `#EFB062` a `src/assets/featured/featured-*.png`; regenerar amb `npm run assets:storyset-featured`. Atribució Storyset al peu (`SiteFooter`). Després de canviar `data/seed-content.json`: `npm run db:seed`.
