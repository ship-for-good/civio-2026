...
describe("classify — grafo v2 (keywords.json)", () => {
  describe("contratacion", () => {
    ...
    it("Given licitación, Then topicId es contratacion", () => {
      expect(classify("licitación de obras en el municipio").topicId).toBe("contratacion");
    });
  });
  ...
  describe("subvenciones", () => {
    ...
    it("Given ayudas o becas, Then topicId es subvenciones", () => {
      expect(classify("ayudas para autónomos en 2024").topicId).toBe("subvenciones");
      expect(classify("beca de investigación del ministerio").topicId).toBe("subvenciones");
    });
  });

  describe("retribuciones", () => {
    it("Given sueldos o ministro, Then topicId es retribuciones", () => {
      expect(classify("cuánto cobra un ministro").topicId).toBe("retribuciones");
      expect(classify("sueldo del alcalde de Sevilla").topicId).toBe("retribuciones");
      expect(classify("salario de los diputados").topicId).toBe("retribuciones");
    });
  ...
  describe("bienes_patrimonio", () => {
    it("Given declaración de bienes o patrimonio, Then topicId es bienes_patrimonio", () => {
      expect(classify("declaración de bienes de un diputado").topicId).toBe("bienes_patrimonio");
      expect(classify("patrimonio del presidente del gobierno").topicId).toBe("bienes_patrimonio");
    });
  });

  describe("normativa_boe", () => {
    it("Given ley o real decreto, Then topicId es normativa_boe", () => {
      expect(classify("real decreto sobre transparencia").topicId).toBe("normativa_boe");
    });
  });

  describe("derecho_acceso", () => {
    it("Given solicitud de acceso, Then topicId es derecho_acceso", () => {
      expect(classify("pedir documentos al ministerio por derecho de acceso").topicId).toBe(
        "derecho_acceso"
      );
    });
  });
  ...
  describe("unknown", () => {
    ...
    it("consultas solo ambientales caen en unknown (fuera del grafo v2)", () => {
      ...
      expect(classify("contaminación del río en mi municipio").topicId).toBe("unknown");
    });
  });
  ...
  describe("Prioridad del grafo", () => {
    ...
    it("declaración antes que normativa genérica", () => {
      expect(classify("declaración de bienes").topicId).toBe("bienes_patrimonio");
    });
  });
});
